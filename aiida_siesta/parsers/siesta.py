...
def is_variable_geometry(xmldoc):
    """Tries to guess whether the calculation involves changes in geometry."""
...
class SiestaParser(Parser):
    ...
    def _get_output_nodes(self, output_path, messages_path, xml_path,
        ...
        parser_version = 'aiida-0.9.1--plugin-0.9.7'
        parser_info = {}
    ...
    def _fetch_output_files(self, retrieved):
        ...
        if self._calc._DEFAULT_XML_FILE in list_of_files:
            xml_path = os.path.join(
                out_folder.get_abs_path('.'), self._calc._DEFAULT_XML_FILE)
